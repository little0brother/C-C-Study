...
2. 同类型结构体变量是否可以相互赋值？会存在什么风险？
	答：会出现浅拷贝或深拷贝的问题，浅拷贝可能会导致连续两次释放同一块内存空间，导致内存出错。
...
6. 分析以下结构体所占空间大小
	...
	#pragma pack(2)     //2字节对齐		2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 = 18
	#pragma pack(4)     //4字节对齐		4 + 4 + 4 + 4 + 4 = 20
	#pragma pack(8)     //8字节对齐		8 + 8 + 8 = 24
	#pragma pack(16)    //16字节对齐		16 + 16 (放不满还是24) = 24
